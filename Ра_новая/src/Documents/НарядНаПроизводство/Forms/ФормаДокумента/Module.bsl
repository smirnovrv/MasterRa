...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ТарифЦеха1) Тогда
		...
		Если ВидРабот = Перечисления.ВидыРабот.СобственнаяПечатьНестандартныйРазмер Тогда 
			Если ВидМашины = Перечисления.ВидыПечатныхМашин.Интерьерная Тогда  
				Объект.ТарифЦеха1 = 16; //Сделать авт
			ИначеЕсли ВидМашины = Перечисления.ВидыПечатныхМашин.Широкоформатная Тогда
				Объект.ТарифЦеха1 = 13;	//Сделать авт 
			КонецЕсли;
		Иначе
			Объект.ТарифЦеха1 = ОбщегоНазначенияВызовСервера.ПолучитьТарифЦеха(ВидРабот);			
		КонецЕсли;
	КонецЕсли;
	...
	Если СоставВидаРабот.МассивТарифов.Количество() > 0 Тогда
		Сч = 1;
		Для Каждого стрМассив Из СоставВидаРабот.МассивТарифов Цикл
			ПараметрТариф = "ТарифЦеха"+Сч; 
			Объект[ПараметрТариф] = стрМассив.Тариф;
			Сч = Сч + 1;
		КонецЦикла;	
		
		Элементы.ТарифЦеха2.Видимость = Объект.ТарифЦеха2;
		Элементы.ТарифЦеха3.Видимость = Объект.ТарифЦеха3;
	Иначе
...
Процедура РассчитатьФЗП()
	...
	Тариф = ?(ЗначениеЗаполнено(Объект.ТарифЦеха1),Объект.ТарифЦеха1,Выборка.Тариф); 
