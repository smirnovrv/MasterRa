...
Процедура ПечатьСчетаФактуры(ТабДок, ПараметрКоманды) Экспорт 
	...
	Если СчетФактура451 Тогда
		...
		ОбластьМакета.Параметры.ИННпоставщика           = ПараметрКоманды.Организация.ИНН + "/" + ПараметрКоманды.Организация.КПП;
	...
	Если СчетФактура451 Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "СУММА(АктТовары.Сумма) КАК Стоимость",
			"СУММА(АктТовары.Сумма) КАК СуммаБезНДС");
	КонецЕсли;
